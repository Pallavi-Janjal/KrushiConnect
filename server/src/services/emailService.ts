...
const buildEmailHtml = (otp: string): string => `
  <!DOCTYPE html>
  <html>
  <head>
    <meta charset="utf-8">
 ...
 */
const sendViaBrevoApi = async (apiKey: string, toEmail: string, otp: string): Promise<EmailResult> => {
  try {
 ...
 */
const sendViaResendApi = async (apiKey: string, toEmail: string, otp: string): Promise<EmailResult> => {
  try {
