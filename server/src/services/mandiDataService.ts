...
export interface MandiRecord {
  ...
  updatedAt: string;
  isDemo: boolean;
  trendHistory: Array<{ date: string; price: number }>;
}

// Major APMC markets by district for Maharashtra
export const MAHARASHTRA_DISTRICT_APMCS: Record<string, string[]> = {
...
// Commodity benchmarks with typical base modal price in ₹/quintal
export interface CropProfile {
  name: string;
  variety?: string;
  basePrice: number;
  volatility: number; // percentage price swing range
  unit?: string;
}

export const COMMODITY_PROFILES: CropProfile[] = [
...
function generateTrend(modalPrice: number, seed: string): Array<{ date: string; price: number }> {
...
export function generateMandiRecordsForLocation(
  ...
  if (districtName && districtName !== 'ALL') {
    ...
    targetDistricts = Object.keys(MAHARASHTRA_DISTRICT_APMCS);
  } else {
  ...
  for (const dist of targetDistricts) {
    ...
    const apmcList = (isMaharashtra && MAHARASHTRA_DISTRICT_APMCS[dist]) 
