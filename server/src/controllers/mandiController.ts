...
import { generateMandiRecordsForLocation } from '../services/mandiDataService';
  ...
  try {
    ...
    if (liveRecords.length > 0) {
      ...
      if (requestedSearch) {
        ...
        filteredRecords = liveRecords.filter((r: any) =>
    ...
    const fallback = generateMandiRecordsForLocation('Maharashtra');
    res.json({
