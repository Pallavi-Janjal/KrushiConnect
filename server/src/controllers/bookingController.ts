...
export const getOwnerBookings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const list = await Booking.find({ ownerId: req.user.userId }).sort({ createdAt: -1 });
    res.json(list.map(b => b.toJSON()));
  } catch (error: any) {
...
export const requestCompletionOtp = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
export const verifyOtpAndComplete = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
